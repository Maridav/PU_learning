...
Positive-Unlabeled Learning Using a Traditional Classifier
"""
...
def create_set_gauss(var = 0.2, num_of_centers = 20, prob_of_labeled = 0.3):
    ...
    sample_points0 = np.array([sample_point(mean0, var * np.identity(1)) for _ in range(100)])
    sample_points1 = np.array([sample_point(mean1, var * np.identity(1)) for _ in range(200)])
